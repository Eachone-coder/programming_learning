...
class Pscws
{
     ...
     */
    public function Pscws($charset = 'utf8')
    {
        $this->_xd = false;
        $this->_rs = $this->_rd = [];
     ...
     * 设置规则集
     *
     * @param string $path 规则集路径
     */
    public function set_rule($path)
    {
        $this->_rule_load($path);
    }
     ...
     * @return bool
     */
    public function _rule_load($path)
    {
        if (!($fd = fopen($path, 'r'))) {
            return false;
        }
            ...
            for ($i = 0; $i < $zlen; $i++) {
                ...
                if (($r1['flag'] & PSCWS4_ZRULE_PREFIX) && ($i < ($zlen - $clen))) {
                    ...
                    $wmap[$i][$k] = [
                        'tf' => $r1['tf'],
                        'idf' => $r1['idf'],
                        'flag' => (PSCWS4_WORD_RULE | PSCWS4_WORD_FULL),
                        'attr' => $r1['attr'],
                    ];
